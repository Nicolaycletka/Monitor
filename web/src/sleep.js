...
/*
 * Источников три, а не четыре: контрольная таблица оставлена одна.
 *
 * Смысл сравнения — проверить, не точнее ли чужая таблица нашей на
 * ЭТОМ ребёнке. Для этого достаточно одной внешней: вторая давала
 * второе мнение того же рода, но каждый источник back-тестируется по
 * хвосту истории внутри `predictNext`, то есть стоит времени на
 * каждом пересчёте. Huckleberry оставлена как более широко
 * используемая и с более подробной сеткой по возрасту.
 */
export const SOURCES = ["model", "app", "huck"];
...
/*
 * Вес личной прямой = n/(n+FIT_SHRINK). При 6 снах — половина.
 *
 * Было 12, стало 6 — по результатам прогона на реальных данных, и это
 * единственная правка алгоритма за всю серию проверок, под которой
 * есть подтверждение.
 *
 * ЧТО ИСПРАВЛЯЕТ. При осторожном весе прогноз оставался систематически
 * РАННИМ: ребёнок засыпал позже предсказанного в 39 случаях из 60,
 * медиана +9 минут, знаковый критерий z = 2.19. После правки 33 из 60,
 * медиана +4, z = 0.65 — сдвиг перестаёт быть отличимым от нуля.
 *
 * Оговорка о проверке: замер делался на двух выгрузках, но они НЕ
 * независимы — вторая почти целиком включает первую, это один ребёнок
 * и почти одни и те же наблюдения. Так что z = 2.19 стоит само по
 * себе, а не подтверждено повторением.
 *
 * Механизм понятен из чисел: голые таблицы для этого ребёнка
 * промахиваются на +36 и +31 минуту, он бодрствует заметно дольше
 * табличных норм. Усадка тянет прогноз обратно К ТАБЛИЦЕ, то есть
 * ровно туда, где ошибка и живёт. Чем сильнее усадка, тем больше
 * остаточного сдвига: при 24 он +13, при 12 +9, при 6 +4, при 0 ноль.
 *
 * ЧЕГО НЕ ИСПРАВЛЯЕТ. Случайный разброс ребёнка (около ±20 минут)
 * правкой не убрать — убирается только направленная составляющая.
 * Расстояние до факта не меняется вовсе: парное сравнение даёт
 * ровно ноль (21/22 и 22/24, z ≈ 0).
 *
 * Сводные проценты попаданий при этом выглядят хуже (42% против 47%),
 * и это ЛОВУШКА, в которую легко попасть. Критерий Макнемара по
 * расходящимся случаям: попал только смелый вариант — 1 раз, только
 * осторожный — 4, z = 0.89. Вся разница в пять процентных пунктов
 * стоит на трёх наблюдениях из шестидесяти. Сравнивать надо
 * расходящиеся пары, а не проценты.
 *
 * ПОЧЕМУ НЕ 0. Усадка защищает семьи, у которых наблюдений мало: по
 * трём снам личная прямая — это шум, и доверять ей целиком нельзя. При
 * 6 защита слабее, но остаётся: на трёх наблюдениях вес 0.33, на шести
 * половина. Проверено, что на первой трети истории (когда данных
 * меньше всего) хуже не становится: медиана ошибки та же, парно
 * смелее лучше в 9 случаях против 5.
 *
 * ОГОВОРКА. Настроено на одном ребёнке и 60 наблюдениях. Для другого
 * ребёнка, чьи окна бодрствования ближе к табличным, выигрыша может не
 * быть вовсе — там и сдвига нет. Вреда тоже ждать неоткуда: при
 * совпадении с таблицей личная прямая просто повторит её.
 */
export const FIT_SHRINK = 6;
