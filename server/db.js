...
/*
 * Полная строка семьи. Нужна там, где важны ВСЕ поля: выгрузка
 * резервной копии, сверка дат.
 *
 * Отдельный запрос, потому что findHouseholdById намеренно узкий — он
 * зовётся в auth на каждом запросе. Но именно эта узость уже трижды в
 * этом проекте приводила к молчаливым ошибкам: не выбрали охранные
 * колонки — не уходили пуши о вехах; не написали due_at в INSERT —
 * терялась ПДР; взяли birth из узкой строки — в резервную копию не
 * попала дата рождения, и копия стала непригодной для восстановления,
 * оставаясь на вид целой.
 *
 * Правило: если из строки нужно больше, чем id и name, берите ЭТОТ
 * запрос, а не req.household.
 */
export const householdFull = db.prepare("SELECT * FROM households WHERE id = ?");
